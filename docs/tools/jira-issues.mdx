...
### Get Smart Checklist Property

Get the legacy Smart Checklist issue property from a Jira issue.
...
Get the Smart Checklist custom field value from a Jira issue.

The checklist value is read from the custom field directly after the configured
one (configured field ID + 1). For example, with
`JIRA_SMART_CHECKLIST_FIELD_ID=customfield_18600` (the default), the value is
read from `customfield_18601`.
...
Updates go through the Railsware Smart Checklist REST API — **not** the Jira
custom field. The `checklistId` is extracted from the configured custom field
(default `customfield_18600`), then the checklist is replaced via
`PUT rest/railsware/1.0/checklist/{checklistId}/item` with `isReplace: true`,
so the provided string always replaces the entire checklist.
...
|-----------|------|----------|-------------|
| `issue_key` | `string` | Yes | Jira issue key (e.g., 'PROJ-123') |
| `checklist` | `string` | Yes | Smart Checklist markdown string, e.g. `- ToDo List\n+ Checked\nx Skipped\n~ In Progress\n` |
