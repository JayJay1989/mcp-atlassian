...
Requires `JIRA_SMART_CHECKLIST_FIELD_ID` to be set to the Jira custom field ID
for the Smart Checklist `Checklists` field.
...
Requires `JIRA_SMART_CHECKLIST_FIELD_ID` to be set. Disabled when
`READ_ONLY_MODE=true`.
