...
docker run -i --rm \
  ...
  -e JIRA_SMART_CHECKLIST_FIELD_ID=customfield_10001 \
...
<Note>
`JIRA_SMART_CHECKLIST_FIELD_ID` is required only when using the Smart Checklist
tools. Set it to the Jira custom field ID for the Smart Checklist `Checklists`
field, such as `customfield_10001`.
</Note>
