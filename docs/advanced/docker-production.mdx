...
docker run -i --rm \
  ...
  -e JIRA_SMART_CHECKLIST_FIELD_ID=customfield_18600 \
...
<Note>
`JIRA_SMART_CHECKLIST_FIELD_ID` defaults to `customfield_18600`. Override it
only if your Jira instance uses a different custom field ID for the Smart
Checklist `Checklists` field. The readable checklist value lives in field ID
+ 1 (e.g. `customfield_18601`); updates go through the Railsware REST API.
</Note>
